...
oncoprint <- function(M, keys=list(somatic="MUT", germline="GERMLINE", amp="AMP", 
  ...
  square_w <- .9
  square_h <- .4
  ggplot(background.m, aes(x=patient, y=gene_y)) + geom_tile(fill="#DCDCDC", colour="white", size=1.1) + 
    ...
    geom_tile(data=data$splicing, aes(x=patient, y=gene_y), inherit.aes=FALSE, width=square_w, height=.95, fill="#2F4F4F") + 
